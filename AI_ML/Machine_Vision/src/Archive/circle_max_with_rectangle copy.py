...
def detect_circles(frame):
    ...
    for idx, centroids in circle_centroids.items():
        if len(centroids) == avgSamples:
            ...
            #print(f"Circle {idx}: Centroid - X: {avg_x}, Y: {avg_y}, Max Diameter: {max_diameter}")
    return max_diameter
...
while True:
    ...
    # Circle detection
    max_diam = detect_circles(frame)

    # Rectangle detection
    rect = detect_rectangles(frame)

    if rect and max_diam > 0:
        calculate_circles_in_rectangle(rect, max_diam)
